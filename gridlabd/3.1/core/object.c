...
{
	int result=0;
    output_error("[!]object_set_value_by_addr( %x, %x, %s, %x )", obj, addr, value, prop);
							 ...
							 char *value) /**< the value to set */
{
    ...
    output_error("[!]object_set_value_by_name(%s, %s, %s)", obj, name, value);
    output_error("[!]prop = %s at %x",prop, prop);
	if(prop==NULL)
	{
    ...
    output_debug("Trying to set value of %s in %s", prop->name, obj->oclass->name);
    //int object_isa(OBJECT *obj, char *type);
    if(object_isa(obj, "switch") && strcmp(name, "status") == 0)
    {
        //int object_get_value_by_name(OBJECT *obj, PROPERTYNAME name, char *value, int size);
       	//unsigned char *phases;
        //int bytes_copied = object_get_value_by_name(obj,"phases", phases, sizeof(char));

        //output_debug("Trying to set value of switch %s with phases %c, in %s", prop->name, phases, obj->oclass->name);

        //EXPORT int reliability_operation(OBJECT *thisobj, unsigned char desired_phases)
        addr = class_get_function(obj->oclass->name, "change_switch_state");  // this is the magic
        unsigned char phases;
        
        if (strcmp(value,"OPEN") == 0){
            phases = 0x04 | 0x01 | 0x02;
            (*addr)(obj, phases, false);
            output_debug("Set value: %s, with function change_switch_state  at  %x", value, addr);
            return 1;
        } else if (strcmp(value,"CLOSED2") == 0){
            bool pA, pB, pC;
            int val=0;
            phases = 0x00;
            //phase_A_state
            addr = (void*)((char *)(obj+1)+(int64)(prop->addr)); /* warning: cast from pointer to integer of different size */
	        object_set_value_by_addr(obj,addr,value,prop);


            addr = class_get_function(obj->oclass->name, "change_switch_state");  // this is the magic
            output_debug("Set value: %s, with function change_switch_state  at  %x", value, addr);

            val = object_get_value_by_name(obj,"phase_A_state", &pA, sizeof(bool));
            output_debug("Bytes copied to pA: %d",val);
            val = object_get_value_by_name(obj,"phase_B_state", &pB, sizeof(bool));
            output_debug("Bytes copied to pB: %d",val);
            val = object_get_value_by_name(obj,"phase_C_state", &pC, sizeof(bool));
            output_debug("Bytes copied to pC: %d",val);
            if ((bool)pA){ phases |= 0x04; }
            if ((bool)pB){ phases |= 0x02; }
            if ((bool)pC){ phases |= 0x01; }                        
            output_debug("OK Calculated switch phases to be: 0x0%x for obj:%x\n", phases,obj);
            (*addr)(obj, (unsigned char) phases, true);
            output_debug("Set value: %s, with function change_switch_state  at  %x", value, addr);
            return 1;
        }       
    }

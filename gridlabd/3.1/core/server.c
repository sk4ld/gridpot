	...
	while (!shutdown_server)
	{
		...
		else if ((int)newsockfd > 0)
		{
			...
			if ( pthread_create(&thread_id,NULL, http_response,(void*)newsockfd)!=0 ){
				output_error("[!] FIRST CHANCE ERROR WITH pthread_create and the http_response");
				sleep(1);
				if ( pthread_create(&thread_id,NULL, http_response,(void*)newsockfd)!=0 ){
					output_error("[!] SECOND CHANCE ERROR WITH pthread_create and the http_response");
					sleep(1);
					if ( pthread_create(&thread_id,NULL, http_response,(void*)newsockfd)!=0 ){
						output_error("[!] MAJOR ERROR WITH pthread_create and the http_response");
						output_error("unable to start http response thread:%d", errno);
					}
				}
			}
...
#ifdef NEVER
		{
			handleRequest(newsockfd); // ignore this
#ifdef WIN32
...
Next:
	...
	output_verbose("socket %d closed",http->s); // swapped lines, b/c http_close will free(http)
	http_close(http);
	return 0;
}

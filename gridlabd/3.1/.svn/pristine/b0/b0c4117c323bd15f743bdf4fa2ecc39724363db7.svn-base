...
int inverter::init(OBJECT *parent)
{
	...
	if (deltamode_inclusive)
	{
		...
		else
		{
			...
			gl_warning("inverter:%s deltamode code is extremely experimental -- Use at your own risk!",obj->name?obj->name:"unnamed");
			/*  TROUBLESHOOT
			The inverter has the initial implementation of deltamode-compliant, transient simulation capabilities included.  This code is
			not tested very thoroughly and known to be unstable.  It is included to provide a starting point for others interested in
			this type of implementation.  Extreme caution is recommended for its usage and utilizing the results.
			*/
		}
	}
	omega_nominal = f_nominal*2*PI;
